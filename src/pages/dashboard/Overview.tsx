...
const Overview = () => {
  ...
  const stats = [
    ...
    { label: "Credits used", value: `${user?.credits.used.toLocaleString()} / ${user?.credits.total.toLocaleString()}`, delta: `${creditPct}%`, icon: Sparkles },
    { label: "Team members", value: "1", delta: "online", icon: UserSquare2 },
  ];
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 sm:space-y-10 max-w-7xl">
      ...
      <div className="relative overflow-hidden rounded-2xl glass-strong p-6 sm:p-8 bg-nebula">
        ...
        <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            <p className="text-xs uppercase tracking-widest text-primary-glow mb-2">
              {getGreeting()}, {user?.name ?? "Studio"}
            </p>
            <h1 className="font-display text-3xl sm:text-4xl md:text-5xl">
              Ready to make something <em className="text-aurora not-italic">extraordinary?</em>
            </h1>
          </div>
          <Link
            to="/app/video"
            className="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-gradient-primary text-primary-foreground shadow-glow text-sm shrink-0"
          >
          ...
          {recent.map((r) => (
            <div key={r.name} className="flex items-center gap-4 p-4 hover:bg-muted/20 transition cursor-pointer">
              ...
              <span className={`text-xs px-2.5 py-1 rounded-full ${
                ...
                {r.status === "Rendering" && (
                  <span className="inline-block h-1.5 w-1.5 rounded-full bg-primary-glow mr-1.5 animate-pulse" />
                )}
                {r.status}
              </span>
            </div>
