...
import {
  ...
  FolderOpen, Sparkles, CreditCard, Settings as SettingsIcon, Search, Bell, Plus,
} from "lucide-react";
...
const DashboardLayout = () => {
  ...
  return (
    <div className="min-h-screen flex bg-background">
      {/* Sidebar */}
      <aside className="w-64 border-r border-border/60 flex flex-col bg-sidebar shrink-0 sticky top-0 h-screen">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 border-b border-border/60 px-6 flex items-center gap-4 sticky top-0 bg-background/80 backdrop-blur-xl z-30">
          <div className="relative flex-1 max-w-md">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Search projects, prompts, templates…" className="pl-9 bg-muted/30 border-border/60" />
          </div>
